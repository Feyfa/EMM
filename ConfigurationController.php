...
{   
    ...
    public function rolemodule_show(Request $request,$GetType='',$CompanyID='',$ID='') {
        if($GetType == 'getrole') {
            ...
            // root payment term for new agencies
    ...
    public function getReportAnalytic(Request $request) {
        ...
        $rpanalytics = ReportAnalytic::select('report_analytics.leadspeek_type',DB::raw('SUM(pixelfire) as pixelfire'),DB::raw('SUM(towerpostal) as towerpostal'),DB::raw('SUM(bigbdmemail) as bigbdmemail'),DB::raw('SUM(bigbdmpii) as bigbdmpii'),DB::raw('SUM(endatoenrichment) as endatoenrichment'),
                                        ...
                                        DB::raw('SUM(notserve) as notserve'),DB::raw('"0" as platformfee'),DB::raw('SUM(report_analytics.leadspeek_api_id) as activecampaign'),DB::raw('SUM(report_analytics.bigbdmhems)'),DB::raw('SUM(report_analytics.bigbdmtotalleads)'),DB::raw('SUM(report_analytics.bigbdmremainingleads)'),DB::raw('SUM(report_analytics.getleadfailed)'),DB::raw('SUM(report_analytics.getleadfailed_bigbdmmd5)'),DB::raw('SUM(report_analytics.getleadfailed_gettowerdata)'),DB::raw('SUM(report_analytics.getleadfailed_bigbdmpii)'));
